...
class PollerService {
    ...
    async broadcast(content) {
        ...
        for (const target of targets) {
            ...
            if (!bot) {
                this.logger.warn('找不到推送机器人：platform=%s selfId=%s channelId=%s', target.platform, target.selfId || '(任意)', target.channelId);
                continue;
            }
            try {
                ...
                this.logger.error('推送失败：platform=%s selfId=%s channelId=%s error=%s', target.platform, bot.selfId, target.channelId, formatError(error));
            }
        }
    }
    findBot(target) {
