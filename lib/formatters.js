...
function formatOnAirItem(item) {
    return {
        title: (0, types_1.asString)(item.title) || '(無標題)',
...
function formatVideoDetail(response, timezone, now = new Date()) {
    ...
    const title = cleanTitle((0, types_1.asString)(video.title ?? anime.title) || '(無標題)');
    ...
    if ((0, types_1.asString)(video.quality))
        lines.push(`畫質：${(0, types_1.asString)(video.quality)}`);
    if ((0, types_1.asString)(anime.upload_time))
        lines.push(`更新時間：${(0, types_1.asString)(anime.upload_time)}`);
    ...
    if ((0, types_1.asString)(anime.publisher))
        lines.push(`發行：${(0, types_1.asString)(anime.publisher)}`);
    if ((0, types_1.asString)(anime.maker))
        lines.push(`製作：${(0, types_1.asString)(anime.maker)}`);
    if (anime.score !== undefined && anime.score !== null)
        lines.push(`評分：${String(anime.score)}`);
    ...
    if ((0, types_1.asString)(video.rating_desc))
        lines.push(`分級：${(0, types_1.asString)(video.rating_desc)}`);
